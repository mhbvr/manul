...
func main() {
	var (
		...
		runnerCount = flag.Int("runners", 1, "Number of concurrent runners")
	)
	flag.Parse()

	loadTester, err := NewLoadTester(*serverAddr, *maxInflight, &RunnerConfig{*inflight, *mode, *rps, *timeout}, *runnerCount)
	if err != nil {
		log.Fatal(err)
	}
	...
	log.Printf("Initial config: inflight=%d, max-inflight=%d, mode=%s, rps=%.2f, timeout=%v, runners=%d",
		*inflight, *maxInflight, *mode, *rps, *timeout, *runnerCount)
