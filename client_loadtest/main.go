...
func main() {
	var (
		...
		inflight       = flag.Int("inflight", 5, "Current number of in-flight requests")
	...
	loadTester := NewLoadTesterWithConfig(*serverAddr, *maxInflight, *inflight, *mode, *rps, *timeout)
	webHandler := NewWebHandler(loadTester)

	log.Printf("Starting load tester web interface on :%s", *webPort)
